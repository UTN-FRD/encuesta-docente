...
class SendSurveyEmailsCommand extends CConsoleCommand
{
    ...
    const BATCH_SIZE = 40; // Número de correos a enviar por ejecución
    ...
    public function run($args)
    {
        ...
        foreach ($emailsToProcess as $row) {
            ...
            if ($emailSent) {
                $totalSent++;
                echo " -> ENVIADO\n";
                $this->registrarCorreoEnviado($email, $firstname, $subject, $message);
            } else {
                echo " -> ERROR\n";
                $this->registrarCorreoError($email, $firstname, $subject, $message, "Error SMTP/PHPMailer");
            }
...
Si no desea recibir estos correos, responda a este mensaje con el asunto 'BAJA'.";
    ...
    private function registrarCorreoEnviado($email, $username, $asunto, $mensaje)
    {
        $sql = "INSERT INTO correos (destinatario_email, destinatario_nombre, asunto, mensaje, tipo_correo, estado_envio, fecha_envio) 
                VALUES (:email, :username, :asunto, :mensaje, :tipo, 'enviado', NOW())";
    ...
    private function registrarCorreoError($email, $username, $asunto, $mensaje, $error)
    {
        $sql = "INSERT INTO correos (destinatario_email, destinatario_nombre, asunto, mensaje, tipo_correo, estado_envio, error_mensaje, fecha_envio) 
                VALUES (:email, :username, :asunto, :mensaje, :tipo, 'error', :error, NOW())";
